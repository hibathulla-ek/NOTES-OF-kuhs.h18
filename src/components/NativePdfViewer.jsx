...
import { extractGoogleDriveFileId, getGoogleEmbedUrl, getGoogleFileType } from '../lib/driveEmbed'
...
export default function NativePdfViewer({
  ...
  const fileType = getGoogleFileType(driveUrl)
  const isSlides = fileType === 'slides'
  const isDocs = fileType === 'docs'
  const isSlidesOrDocs = isSlides || isDocs

  const pdfUrl = !isSlidesOrDocs && resolvedFileId ? `/api/pdf-proxy?fileId=${encodeURIComponent(resolvedFileId)}` : null
  const embedUrl = getGoogleEmbedUrl(
  ...
  // Google Slides, Google Docs, or manual/automatic iframe fallback viewer
  if (isSlidesOrDocs || (useIframeFallback && embedUrl)) {
    return (
      <div
      ...
      >
        {showToolbar ? (
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 sm:px-4">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center gap-1.5 rounded bg-white px-2.5 py-1 text-slate-700 shadow-sm border border-slate-200">
                {isSlides ? (
                  <>
                    <Presentation className="h-3.5 w-3.5 text-amber-600" aria-hidden="true" />
                    Google Slides
                  </>
                ) : isDocs ? (
                  <>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {driveUrl ? (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-md bg-white border border-slate-300 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-brand-blue focus:outline-none focus:ring-1 focus:ring-brand-accent shadow-sm transition-colors"
                >
        ...
        <iframe
          src={embedUrl}
          title={`${isSlides ? 'Google Slides' : isDocs ? 'Google Docs' : 'Preview'} - ${title}`}
          className={`w-full ${isFullScreen ? 'h-[calc(100vh-2.75rem)]' : 'h-[75vh] sm:h-[80vh]'} border-0`}
          allow="autoplay; fullscreen"
          allowFullScreen={true}
        />

        {/* Accessible fallback bar */}
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 sm:px-4">
          <span>Trouble viewing this {isSlides ? 'presentation' : isDocs ? 'document' : 'file'}?</span>
          {driveUrl ? (
            <a
              href={driveUrl}
              target="_blank"
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {driveUrl ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-md bg-white/10 hover:bg-white/20 px-2.5 py-1 text-xs font-semibold text-white focus:outline-none focus:ring-1 focus:ring-brand-accent transition-colors"
