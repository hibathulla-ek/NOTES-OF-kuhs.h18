...
export default function SearchPage() {
    ...
    async function loadResults() {
      ...
      try {
        ...
        const applyFilters = (builder) => {
          let filteredQuery = builder.eq('is_active', true)

          if (year) {
            filteredQuery = filteredQuery.eq('year', year)
          }

          if (paper) {
            filteredQuery = filteredQuery.eq('paper', paper)
          }

          if (subject) {
            filteredQuery = filteredQuery.eq('subject', subject)
          }

          return filteredQuery.limit(50)
        }
        ...
        if (debouncedQuery) {
          result = await applyFilters(
            supabase.from('notes').select('*').textSearch('search_vector', debouncedQuery),
          )
          ...
          if ((result.data ?? []).length === 0) {
            result = await applyFilters(
              supabase.from('notes').select('*').ilike('title', `%${debouncedQuery}%`),
            )
          }
        } else {
          result = await applyFilters(supabase.from('notes').select('*'))
        }
  ...
  }, [debouncedQuery, year, paper, subject])
  ...
  const resultCountText = debouncedQuery
    ? `Showing ${notes.length} results for "${debouncedQuery}"`
            ...
            {error ? (
              ...
              <div className="flex min-h-80 flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
                ...
                <p className="mt-4 max-w-md text-base font-semibold text-slate-700">
                  No notes found for "{debouncedQuery}". Try a shorter keyword or clear your filters.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
