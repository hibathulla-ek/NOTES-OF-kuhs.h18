...
export default function AdminMCQ() {
  ...
  async function loadItems() {
    ...
    setError('')
    try {
      const data = await adminRequest('/api/admin/mcqs', { password: adminPassword })
      setMcqs(data.mcqs ?? [])
    } catch (err) {
  ...
  async function handleToggle(item) {
    ...
    try {
      ...
      await adminRequest(`/api/admin/mcq?id=${item.id}`, { method: 'PATCH', password: adminPassword, body: { is_active: nextIsActive } })
      toast.success(nextIsActive ? 'MCQ shown' : 'MCQ hidden')
      await loadItems()
    } catch (err) {
  ...
  async function handleDelete(item) {
    ...
    setBusyId(item.id)
    try {
      await adminRequest(`/api/admin/mcq?id=${item.id}`, { method: 'DELETE', password: adminPassword })
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-brand-blue">All MCQs</h1>
          <p className="mt-2 text-sm font-semibold text-slate-600">{mcqs.length} total</p>
        </div>
        <Link to="/admin/mcq/new" className="inline-flex items-center justify-center gap-2 rounded-md bg-brand-blue px-4 py-3 text-sm font-semibold text-white hover:bg-brand-accent">
        ...
        <>
          <div className="grid gap-4 md:hidden">
            {sortedItems.length === 0 ? (
              <div className="p-8 text-center text-slate-500">No MCQs yet.</div>
            ) : (
              sortedItems.map(m => (
                <article key={m.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="space-y-3">
                    ...
                    <h2 className="line-clamp-2 text-base font-bold leading-6 text-slate-950">{m.question}</h2>
                    <div className="flex flex-wrap gap-2 text-xs">
                      <span className="rounded bg-slate-100 px-2 py-1">{m.year}</span>
                      <span className="rounded bg-slate-100 px-2 py-1">{m.paper}</span>
                    </div>
                    <Actions item={m} isBusy={busyId === m.id} onToggle={handleToggle} onDelete={handleDelete} stacked />
                  </div>
                </article>
              ))
            )}
          ...
          <div className="hidden overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm md:block">
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-bold uppercase text-slate-500"><SortHeader label="Question" sortKey="question" sort={sort} onSort={toggleSort} /></th>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {sortedItems.length === 0 ? (
                  <tr><td colSpan="6" className="p-8 text-center text-slate-500">No MCQs yet.</td></tr>
                ) : (
                  sortedItems.map(m => (
                    <tr key={m.id}>
                      <td className="max-w-xs truncate px-4 py-4 text-sm font-semibold text-slate-950">{m.question}</td>
                      <td className="px-4 py-4 text-sm"><SubjectBadge subject={m.subject} /></td>
                      <td className="px-4 py-4 text-sm text-slate-700">{m.year}</td>
                      <td className="px-4 py-4 text-sm text-slate-700">{m.paper}</td>
                      <td className="px-4 py-4"><StatusBadge isActive={m.is_active} /></td>
                      <td className="px-4 py-4"><Actions item={m} isBusy={busyId === m.id} onToggle={handleToggle} onDelete={handleDelete} /></td>
                    </tr>
                  ))
                )}
              </tbody>
